...
# %%
all_data = pd.read_csv('datasets/groceries - groceries.csv')
...
# %%
result_fpgrowth.sort_values('support',ascending=False)
...
# %%
result_fpmax.sort_values('support',ascending=False)
...
# plt.savefig('plots/lab4/barplot.png')


# %%
# нормированна (фактически уровень поддержки)
plt.figure(figsize=(8,6))
...
# %%
result_fpgrowth_f.sort_values('support', ascending=False)
...
for i in end_of_generating_n_length_itemsets:
    plt.axvline(min_sups[i] - 0.005, color='red')
ax.set_xlabel('Уровень поддержки')
ax.set_ylabel('Количество наборов')
plt.legend()
# plt.savefig('plots/lab4/count_of_sets_supp.png')
plt.show()
...
# $support = support(AB)$
# 
# $confidence(A \to B) = \frac{support(AB)}{support(A)} = \frac{rules[support]}{rules[antecedent\ support]}$
# 
# $lift(A \to B) = \frac{confidence(A \to B)}{support(B)} = \frac{rules[confidence]}{rules[consequent\ support]}$
# 
# $levarage(A \to B) = support(A \to B)−support(A) \times support(B) = rules[support] - rules[antecedent\ support]\times rules[consequent\ support]$
# 
# $conviction(A \to B) = \frac{1 − support(B)}{1−confidence(A \to B)} = \frac{1-rules[consequent\ support]}{1-rules[confidence]}$
...
# %%
association_rules_res = association_rules(result_fpgrowth, metric='confidence', min_threshold = 0.33)
association_rules_res


# %%
association_rules(result_fpgrowth, metric='lift', min_threshold = 1.65)


# %%
association_rules(result_fpgrowth, metric='leverage', min_threshold = 0.016).shape


# %%
association_rules(result_fpgrowth, metric='conviction', min_threshold = 1.18).shape
...
nx.draw(
    digraph,
    pos,
    labels={node: str(list(node)) for node in digraph.nodes()},
...
plt.axis('off')
# plt.savefig('plots/lab4/graph.png')
plt.show()
...
sns.heatmap(rules_pivot, cmap='viridis')
plt.tight_layout()
