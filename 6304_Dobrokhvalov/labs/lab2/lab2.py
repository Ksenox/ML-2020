...
for i in range(data.shape[1]-1):
    df_scaled.plot.scatter(x=var_names[i], y=var_names[i+1], c='Type', cmap='plasma', ax=axs[i // 2, i % 2])

if saveFigure:
    plt.savefig(plot_base_path+'lab2_original_scatter.png')
...
if saveFigure:
    plt.savefig(plot_base_path+'lab2_cumvar_original.png')
...
df_pca_fitted.plot.scatter(x='First component', y='Second component', c='Type', cmap='plasma', ax=axs)

if saveFigure:
    plt.savefig(plot_base_path+'lab2_scatter_pca.png')
...
if saveFigure:
    plt.savefig(plot_base_path+'lab2_cumvar_pca_2com.png')
...
if saveFigure:
    plt.savefig(plot_base_path+'lab2_pca_explained_variance_on_count_of_components.png')
...
if saveFigure:
    plt.savefig(plot_base_path+'lab2_cumvar_pca_4com.png')
...
def compare_scatter_plot(first_data, second_data, left_title=None, right_title=None, xlabel=None, ylabel=None):
    ...
    fig.set_figwidth(11)
    fig.set_figheight(5)
    axs[0].scatter(first_data[:,0],first_data[:,1],c=labels,cmap='plasma')
    axs[0].set_title(left_title)
    axs[0].set_xlabel(xlabel)
    axs[0].set_ylabel(ylabel)
    axs[1].scatter(second_data[:,0],second_data[:,1],c=labels,cmap='plasma')
...
if saveFigure:
    plt.savefig(plot_base_path+'lab2_compare_orginal_inversed_Na_K.png')


compare_scatter_plot(data[:, [2,8]], pca_inversed_85[:, [2,8]], 'original', 'restored', 'Mg', 'Fe')
if saveFigure:
    plt.savefig(plot_base_path+'lab2_compare_orginal_inversed_Mg_Fe.png')
...
fig.set_figwidth(16)
fig.set_figheight(5)
axs[0].scatter(pca_full_data[:,0],pca_full_data[:,1],c=labels,cmap='plasma')
...
if saveFigure:
    plt.savefig(plot_base_path+'lab2_compare_pca_svd_solvers.png')
# plt.show()
...
fig.set_figwidth(11)
fig.set_figheight(5)
axs[0].scatter(sparse_pca_lars_data[:,0],sparse_pca_lars_data[:,1],c=labels,cmap='plasma')
axs[0].set_title('lars')
axs[1].scatter(sparse_pca_cd_data[:,0],sparse_pca_cd_data[:,1],c=labels,cmap='plasma')
axs[1].set_title('cd')

if saveFigure:
    plt.savefig(plot_base_path+'lab2_compare_sparse_pca_methods.png')
...
fig.set_figwidth(11)
fig.set_figheight(5)
axs[0].scatter(sparse_pca_lars_alpha_1_data[:,0],sparse_pca_lars_alpha_1_data[:,1],c=labels,cmap='plasma')
axs[0].set_title('aplha=1')
axs[1].scatter(sparse_pca_lars_alpha_03_data[:,0],sparse_pca_lars_alpha_03_data[:,1],c=labels,cmap='plasma')
axs[1].set_title('aplha=0.3')

if saveFigure:
    plt.savefig(plot_base_path+'lab2_compare_sparse_pca_alpha.png')
...
if saveFigure:
    plt.savefig(plot_base_path+'lab2_compare_fa_pca.png')
